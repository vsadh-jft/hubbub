...
def springSecurityService
    ...
    def list() {
        println "paramssssssssssssssssssssssssssssssss" + params
       params.max = Math.min(params.max ? params.int('max') : 10, 100)
        [postInstanceList: Post.findByUser(SUser.get(springSecurityService.principal.id)), postInstanceTotal: Post.count()]
    }
    ...
    def timeline() {
        def user = SUser.get(springSecurityService.principal.id)

        def allPost
        allPost = user.posts
        println "-----------------" + allPost
        def test3 = user.following
        println "test3 is " + test3
        test3.each { allPost.addAll(it.posts) }

        //   user.following*.posts.each {allPost.addAll(it)}
        println "------------------------------------------" + allPost.sort { it.dateCreated }
        allPost = allPost.sort { it.dateCreated }.reverse()
        print "ALL POST IS " + allPost
    ...
    def addPost = {
        def user = SUser.findById(params.id)
        if (user) {
